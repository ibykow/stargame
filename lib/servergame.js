...
(typeof module !== "undefined" && module !== null ? module : {}).exports = ServerGame = (function(superClass) {
  ...
  ServerGame.prototype.getShipStates = function() {
    var j, len, player, ref, results, shipState;
    ref = this.players;
    results = [];
    for (j = 0, len = ref.length; j < len; j++) {
      player = ref[j];
  ...
  ServerGame.prototype.update = function() {
    var i, j, k, len, logEntry, player, players, ref;
    this.newBullets = [];
    for (i = j = 1, ref = Config.server.updatesPerStep; 1 <= ref ? j <= ref : j >= ref; i = 1 <= ref ? ++j : --j) {
      ...
      for (k = 0, len = players.length; k < len; k++) {
        ...
        player.update();
        console.log('updated player', player.id, logEntry != null ? logEntry.sequence : void 0, logEntry != null ? logEntry.inputs : void 0, player.inputSequence, player.ship.position);
      }
