...
(typeof module !== "undefined" && module !== null ? module : {}).exports = ServerGame = (function(superClass) {
  ...
  ServerGame.prototype.preparePlayerInputs = function() {
    var j, len, player, ref, results;
    ref = this.players;
    results = [];
    for (j = 0, len = ref.length; j < len; j++) {
      player = ref[j];
      if (player) {
        if (player.input && player.input.length) {
          player.input.sort(function(a, b) {
            return a.tick.count - b.tick.count;
          });
          results.push(player.input = player.input.reduce((function(p, n) {
  ...
  ServerGame.prototype.step = function(time) {
    ...
    shipStates = this.generateShipStates();
