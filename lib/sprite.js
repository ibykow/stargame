  ...
  Sprite.interpolate = function(prevState, nextState, rate) {
    ...
    return {
      velocity: (function() {
        var j, ref, results;
        results = [];
        for (i = j = 0, ref = prevState.velocity.length; 0 <= ref ? j < ref : j > ref; i = 0 <= ref ? ++j : --j) {
          results.push((nextState.velocity[i] - prevState.velocity[i]) * rate + prevState.velocity[i]);
        }
        return results;
      })(),
      position: (function() {
        var j, ref, results;
        results = [];
        for (i = j = 0, ref = prevState.position.length; 0 <= ref ? j < ref : j > ref; i = 0 <= ref ? ++j : --j) {
          results.push((nextState.position[i] - prevState.position[i]) * rate + prevState.position[i]);
        }
        return results;
      })()
    };
