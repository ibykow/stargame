var Eventable, Sprite, Util, abs, isarr, ref, round, sqrt, trunc,
...
(typeof module !== "undefined" && module !== null ? module : {}).exports = Sprite = (function(superClass) {
  ...
  function Sprite(game, position, width, height, color) {
    ...
    this.color = color;
    Sprite.__super__.constructor.call(this, this.game);
  ...
  Sprite.prototype.distanceTo = function(sprite) {
    return Util.magnitude(this.positionDelta(sprite));
  };

  Sprite.prototype.positionDelta = function(sprite) {
    if (!(sprite != null ? sprite.position.length : void 0)) {
      return [0, 0];
    }
    return Util.toroidalDelta(this.position, sprite.position, this.game.toroidalLimit);
  };
  ...
  Sprite.prototype.intersects = function(sprite) {
    ...
    delta = this.positionDelta(sprite);
  ...
  Sprite.prototype.getState = function() {
    ...
    return Object.assign(Sprite.__super__.getState.call(this), {
  ...
  Sprite.prototype.setState = function(state) {
    Sprite.__super__.setState.call(this, state);
    return this.position = state.position, this.velocity = state.velocity, this.width = state.width, this.height = state.height, this.color = state.color, this.flags = state.flags, this.children = state.children, state;
  };
  ...
  Sprite.prototype.updatePosition = function() {
    var x, y, z;
    x = trunc((this.position[0] + this.velocity[0] + this.game.width) * 100) / 100;
    y = trunc((this.position[1] + this.velocity[1] + this.game.height) * 100) / 100;
    z = trunc((((this.position[2] + Util.TWO_PI) % Util.TWO_PI) * 100) / 100);
    x %= this.game.width;
    y %= this.game.height;
    this.position[0] = x;
