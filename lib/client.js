...
(typeof module !== "undefined" && module !== null ? module : {}).exports = Client = (function() {
  function Client(canvas1) {
    var cb, event, info, key, ref, ref1, ref2;
    this.canvas = canvas1;
    if (!this.canvas) {
      return;
    }
    ...
    this.keymap = new Array(0xFF);
    ref2 = Config.client.keys;
    for (key in ref2) {
      info = ref2[key];
      this.keymap[info.code] = info.action;
    }
    this.keys = (function() {
      var j, results;
      results = [];
  ...
  Client.prototype.getKeyboardInputs = function() {
    var i, j, ref, results;
    results = [];
  ...
  Client.prototype.events = {
    socket: {
      ...
      welcome: function(data) {
        ...
        this.game = new ClientGame(this.canvas, this.socket, data);
        this.game.client = this;
        this.socket.emit('join', this.game.player.name);
        this.game.player.ship.updateView = this.game.player.ship.updateViewMaster;
