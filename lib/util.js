var Util, floor, isarr, max, min, ref, rnd, trunc;

ref = [Math.floor, Array.isArray, Math.min, Math.max, Math.random, Math.trunc], floor = ref[0], isarr = ref[1], min = ref[2], max = ref[3], rnd = ref[4], trunc = ref[5];
...
(typeof module !== "undefined" && module !== null ? module : {}).exports = Util = {
  ...
  areSquareBoundsOverlapped: function(a, b) {
    ...
    for (i = j = 0, ref1 = a[0].length; 0 <= ref1 ? j < ref1 : j > ref1; i = 0 <= ref1 ? ++j : --j) {
      if (a[0][i] < b[0][i]) {
