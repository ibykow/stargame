...
module.exports = Server = (function() {
  ...
  function Server(io) {
    ...
    this.game = new Game(this, (1 << 17) + 1, (1 << 17) + 1, 10000);
    ref = this.events.io;
    for (event in ref) {
      cb = ref[event];
    ...
    io: {
      connection: function(socket) {
        ...
        if (Server.startPosition) {
          player = this.game.newPlayer(socket, [Server.startPosition[0], Server.startPosition[1], 0]);
        } else {
          player = this.game.newPlayer(socket);
        }
        if (Server.startPosition == null) {
